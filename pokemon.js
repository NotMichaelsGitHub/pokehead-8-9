    ...
    if(sets.length > 1){
        ...
        $.when.apply(null, thing)
            .then(function (value) {
                ...
                for (i = 0; i < sets.length; i++) {
                    ...
                    // remove rotated promos
                    if(sets[i] == 'smp'){
                        var promos = [];
                        for(x = 92; x < carddata[i][0].length; x++){
                            promos.push(carddata[i][0][x])
                        }
                        console.log();
                        qwer.push(promos);
                    }
        ...
        $.when(thing)
            .then(function (value) {
                ...
                // removes the nonstandard promos
                if(sets[0] == 'smp'){
                    var promos = [];
                    for(i = 92; i < carddata[0].length; i++){
                        promos.push(carddata[0][i])
                    }
                    filter_one(promos);
                }
                else{
